def call() {
    ...
    pipeline {
     ...
     environment {
        github_token = credentials('GIT_SECRETS')
    }
        parameters { 
        ...
        stage('Create Terraform workspace'){
            ...
            stage('Terraform plan'){
                steps {
                        script {
                            try{
                                sh "terraform plan -var-file='${params.ENVIRONMENT}.tfvars' -var github_token='github_token' -refresh=true -lock=false -no-color -out='${params.ENVIRONMENT}.plan'"
                            } catch (Exception e){
                                echo "Error occurred while running"
                                echo e.getMessage()
                                sh "terraform plan -refresh=true -lock=false -no-color -var github_token='github_token' -out='${params.ENVIRONMENT}.plan'"
                            }
                        }
                        ...
                        script {
                            try {
                                sh """
                                    ...
                                    terraform destroy -var-file=${params.ENVIRONMENT}.tfvars -var  github_token='github_token' -no-color -auto-approve
                                """
                            } catch (Exception e){
                                echo "Error occurred: ${e}"
                                sh "terraform destroy -no-color -var  github_token='github_token' -auto-approve"
                            }
                        }
